...
def reset_robot(default_qpos: np.ndarray) -> np.ndarray:
    ...
    """
    reset_qpos = default_qpos + np.random.uniform(-0.5, 0.5, default_qpos.shape)
    return reset_qpos
...
def reset_target_position(base_pos: np.ndarray) -> np.ndarray:
    ...
    """
    target_pos = base_pos + np.array([np.random.uniform(0.2, 0.4), np.random.uniform(-0.2, 0.2), np.random.uniform(0.1, 0.4)])
    return target_pos
...
def get_obs(qpos: np.ndarray, ee_pos_w: np.ndarray, ee_rot_w: np.ndarray, base_pos_w: np.ndarray, base_rot_w: np.ndarray, target_pos_w: np.ndarray) -> np.ndarray:
    ...
    """
    # Transform end-effector position to base frame: B_r_BE = C_BW * (W_r_E - W_r_B)
    ee_pos_base = base_rot_w.T @ (ee_pos_w - base_pos_w)

    # Transform end-effector rotation to base frame: C_BE = C_BW * C_WE
    ee_rot_base = base_rot_w.T @ ee_rot_w
    
    # Convert ee_rot_base to quaternion and normalize
    ee_quat_base = quat_normalize(rot_mat_to_quat(ee_rot_base))

    # Transform target position to base frame: B_r_BT = C_BW * (W_r_T - W_r_B)
